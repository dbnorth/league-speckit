...
- **FR-012**: `MenuBar` nav items MUST be filtered by the signed-in user's `role` field from the `users` table / `localStorage` `user` payload. An item is shown only when `user.role` is in that item's allowed roles. Feature 1 ships **Sign out** (all authenticated roles) and no catalog links; later features add items (e.g. **Seasons** / **Leagues** for `admin`).
